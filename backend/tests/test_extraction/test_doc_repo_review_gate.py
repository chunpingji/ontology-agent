...
def _seed_candidates(db, fake_engine, monkeypatch):
    ...
    job = ExtractionJob(
        ...
        source_config={"doc_ref": DOC_IRI, "content_ref": "edms://x", "config_id": str(cfg.id)},
        status="pending",
    )
    db.add(job)
    db.commit()
...
def test_unverified_confirm_and_rejected_candidates_never_commit(
    client, analyst_headers, db, fake_engine, monkeypatch
):
    """019：未验证旧候选不能直接提交；拒绝决定仍可保存。"""
    cands = _seed_candidates(db, fake_engine, monkeypatch)
    c_ok, c_no = cands[0], cands[1]

    r = client.put(
        f"/api/extraction/candidates/{c_ok.id}/review",
    ...
    db.refresh(c_ok)
    assert c_ok.committed_iri is None

    r = client.put(
        f"/api/extraction/candidates/{c_no.id}/review",
...
def test_commit_requires_senior_analyst(client, operator_headers, db, fake_engine, monkeypatch):
    ...
    c = cands[0]
    r = client.put(
        f"/api/extraction/candidates/{c.id}/review",
        json={"status": "confirmed"},
...
def test_rejection_is_traceable(client, analyst_headers, db, fake_engine, monkeypatch):
    ...
    entry = (
        db.query(AuditLog)
        .filter(AuditLog.action == "extraction.candidate.review", AuditLog.entity_iri == str(c.id))
