...
def _doc_job(db, cfg, *, content_ref=CONTENT_REF) -> ExtractionJob:
    job = ExtractionJob(
        ...
        source_config={"doc_ref": DOC_IRI, "content_ref": content_ref, "config_id": str(cfg.id)},
        status="pending",
    )
    db.add(job)
    db.commit()
...
def test_doc_repo_branch_reuses_alignment_grouping_degraded(db, fake_engine, monkeypatch):
    ...
    # 未声明身份属性，不能由业务字段名称拼接身份。
    assert all(c.group_key is None for c in cands)
