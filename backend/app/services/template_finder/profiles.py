"""Isolated Profile interpreter, adapted from b6f9bd4e55822e20dce0a2ef0fa8f6d9fdd472c1.
...
def _parse_source(raw) -> LocatorProfile:
    ...
    anchor_any = _as_strings(anchors.get("any_of") if isinstance(anchors, dict) else anchors)
    ...
    if locator == "table_singleton" and orientation == "kv":
        if not key_aliases or not value_aliases:
            raise DocumentProfileError("table_singleton kv requires key_aliases and value_aliases")
...
def parse_profile(raw: str | dict) -> DocumentExtractionProfile:
    ...
    return DocumentExtractionProfile(
        ...
        transforms={
            str(key): value for key, value in transforms.items() if isinstance(value, dict)
        },
        identity=_parse_identity(payload.get("identity")),
        subclass_by=(str(payload["subclass_by"]) if payload.get("subclass_by") else None),
        label=str(payload["label"]) if payload.get("label") else None,
        applicability=(
            payload.get("applicability") if isinstance(payload.get("applicability"), dict) else None
        ),
    )
...
def compile_ontology_bindings(
    ...
    for prop in engine.get_data_properties_by_domain(target_class_iri) or []:
        ...
        reject_invalid = bool(
            ...
            or (isinstance(transform_config, dict) and transform_config.get("reject_invalid"))
        )
        bindings.append(
...
def _matches(source: str, alias: str) -> bool:
    ...
    return bool(
        source_norm
        and alias_norm
        and (source_norm == alias_norm or alias_norm in source_norm or source_norm in alias_norm)
    )


def _match_quality(header_norm: str, alias_norm: str) -> tuple[int, int] | None:
...
def _assign_columns(
    ...
    for score, b_idx, header in candidates:
        ...
        if score < binding_best[b_idx]:
            ...
            continue
        avail_cols = [
            c for c in candidates if c[1] == b_idx and c[0] == score and c[2] not in used_cols
        ]
        if len(avail_cols) > 1:
            abstained.add(b_idx)
            continue
        rival_bindings = [
            ...
            if c[2] == header
            and c[0] == score
            and c[1] not in used_bindings
            and c[1] not in abstained
        ]
...
def _headers_match(table: DocTable, groups: tuple[tuple[str, ...], ...]) -> bool:
    return all(
        any(_matches(header, alias) for header in table.headers for alias in group)
        for group in groups
    )
...
def _identity_from_raw(
    ...
    identifier_binding = next((binding for binding in bindings if binding.is_identifier), None)
...
def _equipment_candidate_variants(
    ...
    if len(codes) < 2 or not all(_EQUIPMENT_CODE_RE.fullmatch(code) for code in codes):
        return []
    group = "|".join(codes)
    identifier_iris = {binding.property_iri for binding in bindings if binding.is_identifier}
    variants = []
    for code in codes:
        candidate_values = [
            DocumentValue(
        ...
        for scoped_section in scoped_sections:
            for offset, paragraph in enumerate(scoped_section.paras):
                ...
                source_ref = {
                    ...
                    + len(match.group(2))
                    - len(match.group(2).lstrip()),
                    "span_end": match.end(2) - len(match.group(2)) + len(match.group(2).rstrip()),
                }
                value, note = _transform_value(binding, raw_value, key, source_ref)
                ...
                if note:
                    notes.append(note)
        if values or notes:
            identifier = _candidate_identity(structure, raw, values, identity, bindings, fallback)
            candidates.append(
                DocumentCandidate(
    ...
    for table in structure.tables:
        ...
        for row_pos, row in enumerate(table.rows):
            ...
            for b_idx, binding in enumerate(bindings):
                ...
                value, note = _transform_value(binding, row[key], key, source_ref)
                matched.add(binding.property_iri)
                if value is not None:
                    values.append(value)
                if note:
...
def _read_table_singleton(
    ...
    if source.orientation != "kv":
        ...
        rows = _read_table_rows(structure, target_class_iri, profile, source, bindings, matched)
        return rows[:1]
    ...
    for table in structure.tables:
        ...
        for row_pos, row in enumerate(table.rows):
            ...
            value, note = _transform_value(binding, raw_value, parameter, source_ref)
            matched.add(binding.property_iri)
            if value is not None:
                values.append(value)
            if note:
