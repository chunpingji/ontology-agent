"""DEPRECATED compatibility shim — contamination-risk rule knowledge externalized
(spec 006, T043; rule group ``contamination_risk``, R-CP1~4).
...
the declarative rule layer:
  ...
  • in-code single source ...... ``app/services/reasoning/defaults.py``
                                  (``DEFAULT_DECISION_RULES`` → R-CP1~4)
...
ALL_RULES: list = []  # migrated to defaults.DEFAULT_DECISION_RULES (contamination_risk)
