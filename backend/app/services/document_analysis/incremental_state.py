...
def structural_snapshot(store, run, name, value):
    ...
    key = _key(run, name)
    tree = snapshot_delta(value, cache.get(key))[0]
    cache[key] = tree
...
def encode_incremental_state(store, run, token, payload, *, kind):
    from app.services.document_analysis.state_artifacts import StateIntegrityError, encode_state
    ...
    if head:
        ...
        if prior and prior[0] == (revision, head.content_hash):
            ...
            previous = snapshot_delta(raw)[0]
    tree, operations = snapshot_delta(payload, previous)
    ...
    result = {
        ...
        "baseline": encode_state(store, run, token, tree.value) if baseline else None,
        "operations": [] if baseline else operations,
    }
    cache[_key(run, kind)] = ((revision + 1, content_hash(result)), tree)
    ...
    try:
        while True:
            ...
            if current["depth"] == 0:
                ...
                value = decode_state(store, run, current["baseline"])
                tree = snapshot_delta(value)[0]
                break
            base = current["base"]
        ...
        for item in reversed(chain):
            if item["depth"]:
                ...
                tree = snapshot_delta(value, tree)[0]
