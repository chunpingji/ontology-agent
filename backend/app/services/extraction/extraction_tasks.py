...
class GenericExtractionRunner:
    ...
    def _anchor(
        ...
        try:
            return GenericExtractionRunner._resolve_anchor(span, ir, allowed, contexts)
        except ValueError as exc:
            if not hasattr(exc, "quote_details"):
                exc.quote_details = {
                    "evidence_id": span.evidence_id, "quote": span.text[:240],
    ...
    def _resolve_anchor(
        ...
        if span.context:
            contexts = [GenericExtractionRunner._anchor(
                SpanProposal(evidence_id=span.evidence_id, text=span.context), ir, allowed,
            )]
        local_contexts = [a for a in contexts or [] if a.evidence_id == span.evidence_id]
        if local_contexts:
            clipped = []
    ...
    def _anchors(cls, spans, ir, allowed):
        ...
        for index, span in enumerate(spans):
            try:
                resolved[index] = cls._anchor(span, ir, allowed)
            except ValueError as exc:
                if str(exc) != "ambiguous_source_quote":
                    raise
                pending.append((index, span))
        for index, span in pending:
            resolved[index] = cls._anchor(span, ir, allowed, list(resolved.values()))
