"""Frozen-compatible legacy quality experiment; never writes production facts.
...
This implementation is retained only for replaying historical experiment
protocols. New evaluations use :mod:`app.evaluation.quality_guided_variant`,
which delegates directly to the shared ontology-guided execution core.
"""
...
def build_legacy_quality_guided_variant(
    base, ir, structure=None, *, metadata=None, focus_path=()
):
...
# Historical frozen runtimes keep their own copy of the original source. This
# alias only supports repository-level compatibility tests and the explicit
# ``legacy_quality_guided_summary`` benchmark mode.
build_quality_guided_variant = build_legacy_quality_guided_variant
