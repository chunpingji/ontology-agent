...
- 聚焦用户明确的系统需求、设计目标和验收条件。每项新增能力、状态、存储和流程都须能对应具体需求；历史实现、旧设计、行业背景和通用最佳实践不自动构成新增需求或实施授权。
- **无明确的维护和审计类需求，不得擅自设计和实现可靠性、可服务性、溯源审计、历史回放等能力。** 不得以“生产级”“以后可能需要”“更稳健”为由扩展容灾、自动故障恢复、诊断平台、事件溯源、历史快照或兼容体系。
- 实现明确功能所需的机制采用最小方案，并说明对应哪项需求；不得泛称“必要可靠性”来绕过需求边界。缺少需求依据的能力不纳入本期设计和实现，也不得作为完成已授权任务的前置条件。
- 优先减少组件、数据副本、状态分支和处理步骤。既有模块能满足需求时直接复用，不预建通用框架、扩展接口或多套执行路径。
- 当前关系图谱识别只需支持暂停和继续，不支持历史回退。设计目标为：**一个当前恢复入口、一份权威工作状态、一份最新展示缓存，精排结果和调用账本独立保存。** 状态按业务变更更新，继续时直接加载当前状态；不为未要求的回放保存整套历史执行快照，也不将多份快照合成一个每次全量重写的大对象。
- 本文件其他章节及专题文档中的可靠性、审计、恢复等条款，只在当前已明确的对应需求和执行域内适用，不得扩张为全系统默认新增要求。原文证据核验不等于执行历史回放；专题设计中的建议不自动成为已确认需求。
- 本原则约束设计和实施范围，不授权无关删除既有功能、数据或审计记录。需求变更按用户当前明确指令处理；不借保留旧实现继续扩展未经要求的能力。
...
以下约束按本次明确需求及直接受影响的既有契约适用，不作为新增能力清单。
...
- 本体限定合法类型和谓词；文档结构、摘要、语义相似度用于定位和排序。事实成立必须有可定位核对的原文证明，并核验主体归属、对象角色、关系方向、实体层级、极性与条件。
...
- 页面 GET、刷新或切换展示不能隐式启动模型任务。长任务中需求已明确的运行身份、租约、取消、恢复和增量状态由服务端契约管理，不因任务耗时长就默认补齐这些能力。
...
- 方案写清明确需求、输入输出和验收口径；按本次实际涉及的内容说明状态变化、数据/证明来源、失败处理或缓存失效，不为填齐设计章节而新增机制。重要选择说明收益、代价和适用范围。
...
- 完成后复核本次修改范围与差异，在明确需求和既有契约范围内检查证据权限、身份/版本、否定/条件、双存储、恢复幂等和跨用户隔离等实际受影响边界，不借审查扩大建设范围。
