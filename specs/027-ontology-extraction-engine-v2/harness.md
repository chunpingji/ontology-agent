...
通用模型能力不要求开放 shell、SQL、任意文件或网络工具。标准能力目录有 10 个函数，其中 validate_metric 及 validate_graph 的属性分支仅供控制器调用；关系分支为模型必检工具，Qwen 的工具集合从其余 9 个函数按阶段、恢复模式和已授权引用裁选，再结合受控本体菜单和可定位原文完成语义工作。注册、模型可见与实际执行分别验收。
...
`plan_model_turn()` 在每个模型请求前生成 TurnPlan：tools、answer 或 stop。它读取协调器已经预留的账本余量、当前阶段、可用工具和已确认结果；不反复读取 TaskContext 的初始额度快照，未知结局仍占额度。可用工具已经按模型调用者与阶段/recovery mode 筛选，不含 validate_metric；validate_graph 仅在存在冻结关系时可见，缺少有效结果时强制调用并预留回答额度；finalize 不调用该函数，由控制器直接执行必检。函数不发模型请求，也不持久化另一个计划。
...
候选必须经过引用/菜单检查、冻结、独立核验、数量/单位/SHACL 的适用检查以及 proof/dependency 门，才能构图。validate_metric/validate_graph 的属性分支是 controller-only；关系分支在 verification 必须由模型调用并读取结果，控制器在 finalize 必须核对其有效性和检查结论；模型没有调用属性校准分支的路径，也不能因可选工具额度耗尽而跳过它们。不满足可信前置条件时，返回真实缺口，不生成假定校准值。可信规范化结果在同次 finalize 内传给 SHACL；表示图仅为局部变量，不新增图引用或登记/恢复状态，中途暂停后可重新执行纯本地计算。
...
以上 validate_graph controller-only 约定仅保留于属性值分支。关系在 verification 暴露 validate_graph 并要求模型实际调用、接收对应 function_call_output 后再提供语义判断；有缺项时只允许调用该工具，不同时施加阶段最终回答格式。结果与当前冻结声明、上下文和菜单严格绑定，失败结果也返回模型解释，但不能授权入图。模型跳过工具或预算不足时明确未完成；最终 proof gate 再核对服务端结果。

模型调用与控制器最终接纳沿用现有操作展示，不伪造模型 function_call；引用结果登记在现有 materialized_refs，无新会话、权威状态或回放设施。原文语义判断不能覆盖工具失败，工具通过也不能替代原文证明。
