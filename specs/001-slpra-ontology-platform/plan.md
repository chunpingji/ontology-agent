...
- 复用工作台已加载的类层次和类详情，以标签、注释、IRI 派生筛选树；保留命中节点的祖先，按 IRI 去重计数。输入使用既有 `Input`、`Button`。
- 搜索条件只存于页面状态，清空恢复原树；切换模块保留关键词，忽略旧模块晚返回的数据。搜索树重新展开以展示深层命中。
- 每层在筛选前复制并排序节点：有子节点优先，同组用英文类名做忽略大小写的 A–Z 比较；不修改接口返回的树。
- 校验：定向 ESLint、TypeScript 检查及浏览器交互验证，场景见 quickstart 新增小节。
- 增量设计前后宪章检查：仅增加浏览筛选，不新增接口、依赖、存储或本体写入；沿用现有认证、编辑与图谱联动。
