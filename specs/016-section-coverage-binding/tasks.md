...
- [X] T006 [P] [US1] Rewrite/extend `backend/tests/test_extraction/test_slot_suggester.py`: `test_graph_sourced_section_emits_coverage_not_manual` (S1), `test_declarations_reference_types_never_individuals` (S2), `test_unbindable_position_becomes_unresolved_candidate` (S4), `test_declaration_required_true_by_default` (S5), `test_doc_class_iri_optional_request_still_valid` (S6); **remove/replace** `TestOntologyIRIBinding::test_no_match_sets_llm_extraction` (the deleted exact-match behavior, S3). Per [contracts/suggest-slots-api.md](contracts/suggest-slots-api.md).
...
- [X] T017 [US3] In `frontend/src/components/extraction/template-slot-editor.tsx`, add `CoverageBinding` types + `SectionDef.coverage`, and **STOP both collapse sites**: `suggestionToSlot` (~214–227, remove the `extraction ? : 'manual'` collapse, F1) and the ghost-row badge (~2099–2104, F2). *Design invariant:* the frame never renders a graph-sourced / AI position as a 人工/manual slot — an AI-proposed binding is the amber Coverage Decl card (`ikcOK`, «AI 建议» / «采纳·忽略»), and «人工» appears **only** inside the explicitly non-ontology slot groups (`rXC8T` / `lusM4`). Depends on T016.
