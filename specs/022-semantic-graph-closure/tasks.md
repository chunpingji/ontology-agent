...
## 剪枝与增强检索视图评审修订 — AR（设计待实现）

本轮仅修订[方案](../../docs/剪枝和增强语义检索视图方案.md)与
[自适应检索契约](contracts/adaptive-retrieval.md)，以下均为后续待验收工作。
AR-P0–AR-P5 不继承历史 P0–P5、IP 或 ER 的勾选。AR-P0 未全部闭合前，AR-P1
仅允许不改变准入、调度与运行结果的 instrumentation 原型，禁止实现或启用主动软剪枝。

- [ ] AR-P0-01 冻结六类搜索处置及技术未决转换，拆开 `deferred_record_ids` 的候选/续轮/停止/统计职责；列明 v3 一个 supported 停搜测试保留及 v4 多值、反证和有界继续反例。
- [ ] AR-P0-02 冻结空池、前门非空通过、前置全拒、已评分待处置、无可评分及技术暂停的不同结果；按门阶段水位和 expansion_attempt_id 计轮，为 `prepare_next_epoch=None` 后持久推进/停止编写活性验收，不造空 epoch、不永久等待。
- [ ] AR-P0-03 冻结从 observations 派生的 `AdmissionDecision` schema 与 `accept_semantic` 新校验：committed epoch、精确池子集、依赖、幂等及重复/越池/错版本拒绝；保持 `exact_pool`。
- [ ] AR-P0-04 冻结组 ID 到 record/锚点/task 映射与重叠去重，明确组级命中不授予全成员 coverage；把 H0/H1/H2 锚点级归属丢失列作修复任务。
- [ ] AR-P0-05 冻结 `heuristic-first-v4` 白名单、指纹和仅 v4 的 `resume_state.schema_version=1`；列出 v1/v2/v3 原形状/hash/省略字段兼容测试，v1 仍不支持恢复、v2/v3 保留严格恢复，不默认补字段。
- [ ] AR-P0-06 冻结门评估集合、输入 hash、cache hit/miss、请求/费用引用、决策和游标的持久屏障；核对全拒无 epoch 的审计、失租/回滚/崩溃恢复及固定增量成本。
- [ ] AR-P0-07 同步核心 `contracts.py`、公共 schema/投影、前端类型及调用方的目标字段与测试；保持 `soft_pruned ⊆ unattempted`，复用 `adaptive_search_saturated` 暂停语义并补公开诊断，当前仍有剪枝时不得 `in_scope_complete`。
- [ ] AR-P0-08 冻结 `not_rerankable` 与多视图独立可用性、低分必要证据/否定/条件保护、H0–H3 重激活和停止规则；同时覆盖无 epoch 但仍有技术/预算义务的反例。
...
- [ ] AR-P1-01 完成不改变结果的影子观察/决策与费用审计；AR-P0 通过后再接入持久/恢复/公开字段的正式实现，验证观察不会改变原调度和结果。
- [ ] AR-P1-02 AR-P0 后实施隔离 v4 的六类处置、按门阶段候选集合、有类型空/非空结果、小决策及 accept 校验、旧/新快照、费用屏障和公开诊断；保留旧测试，新增活性/重放/覆盖用例，主动模式仅在对应校准及质量门后启用。
- [ ] AR-P2-01 实施跨 H0–H2 锚点归属和自身/祖先/字段角色视图、分视图完整性与缓存身份，不为上下文命中授予事实权限。
- [ ] AR-P3-01 实施受控兄弟及组级召回、record/task 映射、重叠去重和证据角色保护；核对超预算视图不连带淘汰其他视图。
- [ ] AR-P4-01 在最终查询/视图/模型/策略版本上重新采集影子数据并校准准入阈值；P1 旧视图数据不作为新阈值校准证据。
- [ ] AR-P4-02 实施经校准的前置/后置门、候选上限和触发式扩搜，完成主动剪枝工程/恢复验收，计入门与新增制品自身成本；未通过独立质量门不得默认启用。
