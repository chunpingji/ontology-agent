...
2026-09-14 用户指定新版词汇查询的剪枝阈值为 **−3**，后续按实际情况由人工调整。
`subject-slot-query-v2` 的 trial 从校准文件 `rerank_thresholds.discover` 和
`rerank_thresholds.counterevidence` 读取人工指定的有限数值，不再套用旧查询的 −8 上限，
也不把 −3 写成新的硬上限。仅在双意图分数都严格低于各自阈值且满足完整性与保护条件时
软剪枝；等于阈值保留。人工更新校准文件及其 hash 后仅影响新建运行，旧运行继续使用
冻结值，不自动调参。原 v1 校准仍遵守原 −8 限制，development/pending 状态、enforce
审核门、模型/本体/词汇/视图绑定及组保护不变。详见[本次校准记录](../pruning-query-v2-calibration.md)。
