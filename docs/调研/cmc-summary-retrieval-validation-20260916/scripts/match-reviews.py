"""Match independent source reviews to frozen D candidates, without model calls.
...
for case in cases:
    ...
    for group, originals in (
        ...
        for cid, review in review_by_id.items():
            ...
            if destination == "unaccounted" and result.get("status") == "failed":
                # A saved proposal may precede a failed verification request.
                # It was never finalized, so absence is neither rejection nor acceptance.
                assert result.get("failure_stage") == "verification", (scope, cid)
                assert not any(result.get(key) for key in
                               ("accepted", "observations", "unresolved", "rejected"))
                assert not (scope_root / "verification/proposal.json").exists()
                destination = "not_finalized_model_failure"
...
data = {
    ...
    "unreviewed_scope_status": [
        ...
        for path in sorted((root / "D").glob("*/result.json"))
