...
def execute(report):
    ...
    try:
        ...
        with model_scope(bind=bind, run_id=report['run_id'], task_id='s0-lexical-comparison'):
            ...
            def batched(operation, inputs):
                return [value for offset in range(0, len(inputs), config['batch_size'])
                        for value in operation(inputs[offset:offset+config['batch_size']])]

            record_vectors = batched(model.embed, [views[rid].model_text for rid in ordered_ids])
            ...
            for variant, queries in variants:
                ...
                for query, query_vector in zip(queries, query_vectors, strict=True):
                    ...
                    scores = batched(model.score_pairs, pairs)
