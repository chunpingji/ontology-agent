...
主要拒答/拒绝原因包括 `no_subject/ambiguous_subject/no_value_evidence/unsupported_predicate/cooccurrence_only/unit_unknown/scope_violation/conflicting_values/budget_exceeded/model_unavailable`。`negated/conditional_unresolved` 单独记录为无条件正向投影的阻断原因，不能据此将断言本身标为 `validation=rejected`。有充分证据的否定或条件断言可通过验证、审核和提交，保留其极性与条件；条件尚未证实不等于条件表达本身缺少证据。条件和假设候选不满足无条件 coverage。
...
“药品 A 与设备 E 同时出现在介绍中”没有使用关系证据，不能通过关系绑定验证。“药品 A 未使用设备 E”可形成有证据的否定断言，通过验证、审核后独立提交，但不能产生正向 usesEquipment。“若转产则使用 E”可作为有证据的条件断言审核和提交；只有条件在同一事实快照中获证实或经过明确人工决策，才生成有推导/人工来源的独立正向断言，原条件断言仍保留。

### 10.3 多跳与 scope 依赖

每一跳传递真实父候选、revision、已判定路径及 scope。沿已成立正向关系展开的后续路径，要求前一跳通过绑定校验且满足正向投影条件；验证通过的否定或未解条件断言不能作为这类路径的前置正向边。最终提交仍依赖全路径需要的审核状态。没有父实例时先生成主体任务，不退回“父类 IRI + 全文扫描”。
...
`validation_passed` 表示证据支持候选所表达的完整断言，包括主体、谓词/属性、对象/值、极性、条件和适用范围；它不等于正向关系成立、条件已满足或已经人工审核。否定及条件断言与肯定断言使用相同的来源、绑定和一致性门槛，证据充分且无未决冲突时均可通过。是否允许无条件正向投影另行判断，不能以断言极性直接决定验证失败。所有层的结论、失败原因和版本均持久化；不得用平均分掩盖来源缺失或绑定失败。

### 10.6 报告事实边界

报告事实投影只读取 §12 的已审核且提交成功的快照。已提交的否定或未解条件断言保留其语义，用于相应的否定说明、条件说明或缺口结论，不混入无条件肯定叙述；待审核、失败及冲突项只进入审核/缺口视图。人工新增事实也须经过校验、审核记录及提交，不能仅因“人工输入”绕过快照。
...
4. 所有用于满足正向要求的事实已审核、提交成功、满足正向投影条件，且不存在相关未决冲突；否定或未解条件断言即使已提交，也不能计入正向路径或合格对象数。
...
| `confirmed_absent` | 有验证通过、审核确认且提交成功的否定断言（包括人工来源），证据覆盖该目标的主体、谓词、对象范围及适用时间，且无相关未决冲突；引用否定 assertion ID，按模板政策显示不满足或豁免，不伪造正向边 |
...
否定范围不得扩大：“A 未使用设备 E”只能支持相应对象及适用范围内的不存在结论，不能推出“A 未使用任何设备”。否定候选待审核或提交失败时分别保留 `pending_review/incomplete`，正反证据未决时为 `conflict`，均不得提前标为 `confirmed_absent`。未抽到正向事实和条件尚未满足也不构成不存在证据。
...
上述状态轴适用于肯定、否定和条件断言。有充分证据且满足校验要求的否定断言允许经历 `validation=passed → review=confirmed → commit=succeeded`，发布后可支持 §11.3 的 `confirmed_absent`。有证据的条件断言也可按此链路持久化，无需先证明条件已发生。断言提交资格与无条件正向事实物化资格分开判断，禁止以 `assertion_status=negated/conditional` 为由整体拒绝提交。
...
5. 专门的 `FactCommitService` / `OntologyInstanceWriter` 先写有独立证据的实例和类型，再保存带极性、条件及适用范围的断言；只有满足正向投影条件的断言才物化相应数据属性或对象关系，随后保存逐断言 provenance。否定或条件断言走独立断言存储，不要求生成正向边；不能把属性字典传给元数据 `project_entities()` 代替实例写入。
6. 图写入按 commit ID 使用可重入 upsert。节点/边与事实断言 ID 稳定；断言身份包含极性、条件和适用时间，相同 SPO 且限定条件相同的多份证据作为不同来源附着，不重复造实体或边。否定或条件断言独立保存，禁止直接投影为无条件正向数据属性或对象边。
7. 按提交清单回读应保存的实例、断言、极性/条件和来源，以及获准物化的属性/边；对否定或条件断言检查其未被误投影为无条件正向事实，不能因没有正向边判定提交失败。全部匹配后，在数据库事务中标记提交成功并发布快照；报告只读取已发布快照。
...
- 有充分证据的否定及条件断言可以 `validation=passed`；断言极性和未解条件只阻断无条件正向投影，不触发整体拒绝，也不用于继续展开正向关系路径。
...
- 已提交且范围匹配的否定断言可支持 `confirmed_absent`；待审核、提交失败、范围不匹配或存在未决正反冲突时不能确认不存在。对 E 的否定不扩大为对所有设备的否定。
...
- 否定及条件断言完成验证、审核、提交和来源回放，断言本身可查且不产生无条件正向属性/边；无正向边不会被提交回读误报为失败，重试保持原极性与条件。
...
人工确认仍独立于自动阈值。缺来源、无绑定证据或 scope 越界是断言校验的硬失败，不能被高模型分抵消。明确否定必须阻止对应正向事实的物化；有充分证据的否定断言本身可以通过验证、审核和持久化，并用于支持“不存在”的覆盖结论。条件断言同样可以凭支持条件表达的证据通过验证、审核和持久化，但不得直接物化为无条件正向事实。不能将 `negated/conditional_unresolved` 作为断言本身 `validation=rejected` 的理由；提交和覆盖条件分别见 §12.3、§11.3。校准集选择阈值后在锁定测试一次评估，不按测试结果反复调参。
...
| 合法 anchor 被误当作绑定证明 | 独立 BindingEvidence、竞争主体判定；否定/条件阻断无条件正向投影，有证据的断言仍可审核提交 |
...
| E：共现、否定、条件 | A 和 E 共现、A 未使用 E、若转产则使用 E | 无证据共现不通过绑定；可信否定/条件断言可验证、审核、提交且可回放，不产生无条件正向边；范围匹配的已提交否定支持 confirmed_absent，未解条件不满足无条件 coverage | R02、R05、复审 P2 |
