...
单独对 "250mg" 跑 NER，模型无法区分它是"规格"、"剂量"还是"含量"。

### 3.2 方案

在 `annotate_word()` 的 Pass 1 阶段，对表格数据行的每个单元格，**拼入表头前缀**再送入 NER：

```python
# document_annotator.py:429-433
hdr = headers[ci]  # 如 "规格"
if hdr and cell_text:
    prefix = f"{hdr}："        # "规格："
    all_texts.append(f"{prefix}{cell_text}")  # "规格：250mg"
    prefix_lens.append(len(prefix))           # 记录前缀长度=3
```
...
NER 返回的 span 坐标是基于拼接后文本的。在 Pass 2 组装 tiptap 节点前，需要**减去表头前缀长度**，还原为原始单元格文本内的坐标：

```python
# document_annotator.py:444-452
...
前缀长度 `plen` 仅对数据行非零（表头行和段落均为 0），确保段落标注零影响。
...
这些属性标签（"NOAEL"、"分子式"、"PDE"、"制剂剂型"）天然就是 GLiNER 的零样本标签——告诉模型"在这段文本里找这些东西"。
...
源码位置：[document_annotator.py:135-200](backend/app/services/extraction/document_annotator.py#L135-L200)
...
> **设计决策**：数据属性标签（"NOAEL"、"制剂剂型"…）**不**进入种子标签——它们是字段名
> 而非实体类型。混入种子会导致 GLiNER 把文档中的属性名误检为实体 span（如 "制剂剂型"
> 被检出后经嵌入匹配到 "中药制剂"），且大量英文属性标签稀释中文 NER 注意力。属性标签
> 仅用于阶段三的 per-class GLiNER 标签集（`_property_schema_for_class()`）。

源码位置：[ontology_typer.py:59-91](backend/app/services/extraction/ontology_typer.py#L59-L91)
...
源码位置：[ontology_typer.py:116-177](backend/app/services/extraction/ontology_typer.py#L116-L177)
...
2. `_font_size_heading_level()`：按字号启发式（≥20pt→h1, ≥16pt→h2, ≥14pt→h3）
