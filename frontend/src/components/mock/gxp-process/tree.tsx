...
export function ProcessTree({ steps, operations, selectedId, search, expanded, disabled, onSearch, onExpand, onSelect, onAddStep, onAddOperation, onCollapse }: {
  ...
  return <aside aria-label="工艺步骤导航" className="min-w-0 border-b bg-background p-3 lg:sticky lg:top-0 lg:max-h-[calc(100vh-56px)] lg:overflow-y-auto lg:border-b-0 lg:border-r">
    <div className="mb-3 flex items-center justify-between px-1"><h2 className="text-sm font-semibold">工艺步骤</h2><span className="text-xs text-muted-foreground">{steps.length} 个步骤</span></div>
    ...
    <div className="mb-2 flex items-center justify-between gap-1 px-1 text-[11px] text-muted-foreground"><span>全部 · 系统预设与自定义</span><button type="button" className="shrink-0 rounded px-1 py-1 hover:text-primary focus-visible:ring-1 focus-visible:ring-ring" onClick={onCollapse}>{expanded.length ? "收起全部" : "展开全部"}</button></div>
    <nav aria-label="操作模板库" className="max-h-80 space-y-0.5 overflow-y-auto lg:max-h-none lg:overflow-visible">
      {visible.map((step) => {
    ...
    <div className="mt-4 space-y-1 rounded-md bg-muted/60 p-3"><p className="flex items-center gap-2 text-xs font-medium"><FolderTree className="size-3.5 text-primary" />操作模板库</p><p className="text-[11px] text-muted-foreground">选择操作，配置记录要求与处置规则</p></div>
