...
import { ArrowLeft, ArrowRight, CheckCircle2, ChevronRight, ClipboardCheck, Copy, Download, Eye, FileClock, FileText, Info, LockKeyhole, Save, Scissors, Send, Settings2, Workflow } from "lucide-react";
...
import { SamplingConfiguration, SamplingPreview } from "./sampling-config";
import { BatchGenerator } from "./batch-generator";
import { batchNumberIssue, generateBatchDraft, type BatchDraft } from "./batch";
...
interface Workspace {
  steps: ProcessStep[]; operations: Operation[]; selectedId: string; expanded: string[];
  search: string; snapshots: Snapshot[]; dirty: string[]; batch?: BatchDraft;
}
...
export function GxpProcessMock() {
  const [workspaces, setWorkspaces] = useState<Record<string, Workspace>>(() => ({ template: {
    steps: INITIAL_STEPS, operations: createInitialOperations(), selectedId: "OP11.02",
    expanded: ["OP05", "OP11"], search: "", snapshots: [], dirty: [],
  } }));
  const [workspaceId, setWorkspaceId] = useState("template");
  const workspace = workspaces[workspaceId];
  const { steps, operations, selectedId, expanded, search, snapshots, dirty, batch } = workspace;
  function workspaceSetter<K extends keyof Workspace>(key: K) {
    return (next: SetStateAction<Workspace[K]>) => setWorkspaces((items) => ({
      ...items, [workspaceId]: { ...items[workspaceId], [key]: typeof next === "function" ? next(items[workspaceId][key]) : next },
    }));
  }
  const setSteps = workspaceSetter("steps"), setOperations = workspaceSetter("operations");
  const setSelectedId = workspaceSetter("selectedId"), setExpanded = workspaceSetter("expanded");
  const setSearch = workspaceSetter("search"), setSnapshots = workspaceSetter("snapshots"), setDirty = workspaceSetter("dirty");
  ...
  const sourceClause = batch?.report.operations.find((item) => item.operationId === current.batchSourceOperationId);
  const batchNumbers = Object.values(workspaces).flatMap((item) => item.batch ? [item.batch.batchNumber] : []);
  ...
  function generate(reportId: string, batchNumber: string, selected: string[]) {
    if (!canEdit || batchNumberIssue(batchNumber, batchNumbers)) return;
    const id = crypto.randomUUID();
    const result = generateBatchDraft(reportId, batchNumber, selected, id, new Date().toISOString());
    setWorkspaces((items) => ({ ...items, [id]: {
      ...result, selectedId: result.operations[0].id, expanded: result.steps.map((item) => item.id),
      search: "", snapshots: [], dirty: result.operations.map((item) => item.id),
    } }));
    setWorkspaceId(id);
    setGenerating(false);
    setNotice(`已生成批号 ${result.batch.batchNumber} 的演示草稿，保留 ${result.operations.length} 个操作。可继续编辑配置。`);
  }
  function exportBatch() {
    if (!batch) return;
    const payload = { schemaVersion: 1, kind: "mock-batch-gxp", demo: true, batch, steps, operations, snapshots, dirtyOperationIds: dirty };
    const url = URL.createObjectURL(new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" }));
    const link = document.createElement("a");
    link.href = url;
    link.download = `${batch.batchNumber}-GxP-demo.json`;
    link.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
    setNotice("已导出当前批 GxP 演示配置，包含当前编辑、示例来源与生成时的裁剪清单。");
  }
  ...
  function addStep(name: string) {
    if (!canEdit) return;
    const id = `OP${String(Math.max(0, ...steps.map((item) => Number(item.id.slice(2)))) + 1).padStart(2, "0")}`;
  ...
  function save(submitted = false) {
    if (disabled) return;
    if (issues.length && (submitted || (!current.sampling && !batch))) { setModal({ kind: "validation" }); return; }
    ...
    const version = batch ? `v0.${history.length + 1}` : `v1.${6 + history.length}`;
    ...
    setModal(null);
    setNotice(submitted ? `已模拟提交 ${version}，未发起真实发布审核。刷新页面将恢复示例。` : `Mock 草稿 ${version} 已保存至本次会话，刷新页面将恢复示例。${(current.sampling || batch) && issues.length ? ` 仍有 ${issues.length} 项待完善，模拟提交前须完成校验。` : ""}`);
  }

  const parameterSection = <Section number="01" title="过程参数与记录配置" description="按参数类型设置规程要求" action="添加参数 / 测量对象" disabled={disabled} onAdd={() => setModal({ kind: "parameter", parameter: emptyParameter(crypto.randomUUID()) })}>
  ...
  const eventsSection = <Section number={current.sampling ? "05" : "02"} title="时间与事件配置" description="定义必须记录的事件" action="添加事件" disabled={disabled} onAdd={() => newRule("events")}>
  ...
  const checksSection = <Section number={current.sampling ? "06" : "03"} title="操作检查项目" description="配置规定动作、检查方式及必过条件" action="添加检查项目" disabled={disabled} onAdd={() => newRule("checks")}>
  ...
  const reviewSection = <Section number={current.sampling ? "07" : "04"} title="审核与处置预案" description="定义审核角色、异常响应及恢复条件" action="添加预案" disabled={disabled} onAdd={() => newRule("contingencies")}>
  ...
  const configurationSections = [
    { number: "01", title: "过程参数与记录配置", content: parameterSection },
    { number: "02", title: "时间与事件配置", content: eventsSection },
    { number: "03", title: "操作检查项目", content: checksSection },
    { number: "04", title: "审核与处置预案", content: reviewSection },
  ];
  ...
  return <div className="-m-6 min-w-0 bg-accent/30">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-background px-6 py-3">
      <div className="flex items-center gap-2 text-xs text-muted-foreground"><Workflow className="size-4 text-primary" /><span>原料药工艺</span><ChevronRight className="size-3" /><span className="text-foreground">{batch ? "批号工艺配置" : "通用操作设置"}</span><Badge variant="secondary" className="ml-1">静态 Mock</Badge></div>
    ...
    <header className="flex flex-wrap items-center justify-between gap-4 px-6 py-5">
      <div><div className="flex flex-wrap items-center gap-3"><h1 className="text-xl font-bold tracking-tight">{batch ? `批 GxP 配置 · ${batch.batchNumber}` : "通用工艺操作设置"}</h1><Badge variant="outline" className="border-primary/20 bg-primary/5 text-primary">{lastSaved?.version ?? (batch ? "v0.1" : "v1.5")} {dirty.includes(current.id) ? "草稿 · 未保存" : lastSaved?.submitted ? "待审核（演示）" : "草稿"}</Badge></div><p className="mt-1.5 text-xs text-muted-foreground">{batch ? `${batch.report.product} / ${batch.report.id} / ${batch.report.version} / 批号专属演示草稿` : "通用模板 / 化学合成原料药 / 定义操作、记录要求与审核处置规则"}</p></div>
      <div className="flex flex-wrap items-center gap-2"><span className="mr-1 text-xs text-muted-foreground">演示权限：{canEdit ? "工艺管理员 · 可编辑" : "已授权成员 · 只读"}</span><Button variant="outline" size="sm" onClick={() => setModal({ kind: "permissions" })}><Settings2 />权限设置</Button><Button variant="outline" size="sm" onClick={() => setModal({ kind: "history" })}><FileClock />版本历史</Button><Button size="sm" disabled={!canEdit} onClick={() => setGenerating(true)}><Scissors />生成批号工艺</Button></div>
    </header>
    {Object.keys(workspaces).length > 1 && <section aria-label="工艺工作区" className="mx-6 mb-4 space-y-3 rounded-lg border bg-background p-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 flex-wrap items-center gap-2"><span className="text-xs font-medium">当前配置</span><Select value={workspaceId} onValueChange={switchWorkspace}><SelectTrigger aria-label="切换工艺配置" className="h-8 w-64 max-w-full text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="template">通用工艺操作模板</SelectItem>{Object.entries(workspaces).filter(([, item]) => item.batch).map(([id, item]) => <SelectItem key={id} value={id}>批号 {item.batch!.batchNumber} · {item.batch!.report.product}</SelectItem>)}</SelectContent></Select><Badge variant="secondary">会话草稿 · 刷新清空</Badge></div>{batch && <div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => switchWorkspace("template")}><ArrowLeft />返回通用模板</Button><Button variant="outline" size="sm" onClick={exportBatch}><Download />导出批配置</Button></div>}</div>
      {batch && <div className="space-y-1 text-xs leading-relaxed text-muted-foreground"><p>来源：{batch.report.title} · {batch.report.version} · 静态示例</p><p>生成时保留 {batch.includedOperationIds.length} 个操作，裁剪 {batch.excludedOperationIds.length} 个通用操作；当前共 {operations.length} 个操作。模板与各批草稿独立编辑。</p></div>}
    </section>}
    <div className="grid min-w-0 items-start border-t lg:grid-cols-[280px_minmax(0,1fr)] 2xl:grid-cols-[312px_minmax(0,1fr)]">
      <ProcessTree batchMode={!!batch} steps={steps} operations={operations} selectedId={selectedId} search={search} expanded={expanded} disabled={!canEdit} onSearch={setSearch} onExpand={(id) => setExpanded((ids) => ids.includes(id) ? ids.filter((value) => value !== id) : [...ids, id])} onSelect={(id) => { setSelectedId(id); setNotice(""); }} onAddStep={() => setModal({ kind: "step" })} onAddOperation={() => setModal({ kind: "operation" })} onCollapse={() => setExpanded(expanded.length ? [] : steps.map((item) => item.id))} />
      <div className="min-w-0 space-y-4 p-4 xl:p-6">
        <section aria-label="操作基础属性" className="space-y-3">
          <div className="flex flex-wrap items-start justify-between gap-2"><div><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs text-muted-foreground">{current.id}</span><h2 className="text-lg font-semibold">{current.name || "未命名操作"}</h2><Badge variant="secondary" className={current.enabled ? "bg-primary/10 text-primary" : ""}>{batch ? "批操作" : "操作模板"} · {current.enabled ? "已启用" : "已停用"}</Badge>{current.custom && <Badge variant="outline">自定义</Badge>}</div><p className="mt-2 text-xs leading-relaxed text-muted-foreground">{batch ? `当前为批号 ${batch.batchNumber} 的独立配置，编辑仅影响本批草稿。` : `定义${current.name}的操作要求与记录结构，可复用于适用工艺。预设值可由授权用户按规程调整。`}</p></div><div className="flex gap-2"><Button variant="outline" size="sm" disabled={!canEdit} onClick={() => addOperation(`${current.name}（副本）`, true)}><Copy />复制操作</Button><Button variant="outline" size="sm" disabled={!canEdit} onClick={() => update({ enabled: !current.enabled })}>{current.enabled ? "停用操作" : "启用操作"}</Button></div></div>
        ...
        {batch && <section aria-label="CMC 示例来源" className="space-y-2 rounded-md border border-primary/20 bg-primary/5 p-3 text-xs"><div className="flex flex-wrap items-center gap-2 font-medium"><FileText className="size-4 text-primary" />{sourceClause ? `${batch.report.id} · ${sourceClause.section}` : "本批新增操作"}<Badge variant="outline">示例来源 · 待确认</Badge></div><p className="leading-relaxed text-muted-foreground">{sourceClause?.excerpt ?? "此操作在生成后新增或复制，请补充本批适用依据。"}</p><p className="text-[11px] text-muted-foreground">以上为生成时的示例条款；当前配置可编辑，示例摘录保持原样。</p></section>}

        {current.sampling ? <SamplingConfiguration key={`${workspaceId}:${current.id}`} plan={current.sampling} disabled={disabled} onChange={(sampling) => update({ sampling })} sections={{ "05": eventsSection, "06": checksSection, "07": reviewSection }} /> : current.configurationLayout === "tabs" ? <Tabs key={`${workspaceId}:${current.id}`} defaultValue="01" className="min-w-0">
          <div className="min-w-0 overflow-x-auto rounded-lg border bg-background p-1">
            <TabsList aria-label="操作配置分区" className="h-auto w-max min-w-full justify-start gap-1 bg-transparent p-0">
              {configurationSections.map(({ number, title }) => <TabsTrigger key={number} value={number} className="flex-1 gap-2 px-3 py-2.5 text-xs data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:shadow-none"><span className="font-mono">{number}</span>{title}</TabsTrigger>)}
            </TabsList>
          </div>
          {configurationSections.map(({ number, content }) => <TabsContent key={number} value={number} className="mt-4 min-w-0">{content}</TabsContent>)}
        </Tabs> : <>{parameterSection}
        {eventsSection}
        {checksSection}
        {reviewSection}</>}
    ...
    <Dialog open={modal !== null} onOpenChange={(open) => { if (!open) setModal(null); }}><DialogContent className={modal?.kind === "preview" ? "max-h-[85vh] max-w-3xl overflow-y-auto" : "max-h-[85vh] overflow-y-auto"}>
      ...
      {modal?.kind === "validation" && <><div className="space-y-3 text-sm">{issues.length ? <><p className="font-medium text-destructive">发现 {issues.length} 项待修正配置</p><ul className="list-disc space-y-2 pl-5">{issues.map((issue, index) => <li key={index}>{issue}</li>)}</ul></> : <p className="flex items-center gap-2 text-success"><CheckCircle2 className="size-5" />本地字段校验通过</p>}<p className="text-xs leading-relaxed text-muted-foreground">本次仅检查字段完整性、数值格式、条件要求和引用。规程依据、科学适用性及质量批准仍需审核，未执行生产合规判定。</p>{!current.sampling && !current.parameters.length && <p className="text-xs text-muted-foreground">此操作尚未配置过程参数。</p>}</div><DialogFooter><Button variant="outline" onClick={() => setModal(null)}>返回配置</Button></DialogFooter></>}
      {modal?.kind === "history" && <div className="space-y-3">{history.length ? [...history].reverse().map((snapshot) => <div key={snapshot.id} className="space-y-2 rounded-md border p-3"><div className="flex items-center justify-between text-sm"><span className="font-medium">{snapshot.version} · {snapshot.submitted ? "待审核（演示）" : "草稿"}</span><span className="text-xs text-muted-foreground">{snapshot.time}</span></div><p className="text-xs text-muted-foreground">{snapshot.operation.changeNote}</p></div>) : <p className="py-6 text-center text-sm text-muted-foreground">{batch ? "当前批配置尚未保存版本。" : "本次会话暂无保存版本。v1.5 为画板示例草稿。"}</p>}<p className="text-xs text-muted-foreground">版本仅保留在本次页面会话中，刷新后清除。</p></div>}
